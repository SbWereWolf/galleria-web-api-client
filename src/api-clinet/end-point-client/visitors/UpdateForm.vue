...
async function UpdateAddress() {
    ...
    const response = await fetch(
        `${apiLocation.value}${endPoint.value}?session_id=${sessionId}&new_residence=${data.input}`,
        {
            method: method.value,
            headers: headers,
        },
...
<template>
    <div>
        <form @submit.prevent="UpdateAddress">
            <h1>{{ endPoint }}</h1>
            <div>
                <label for="new_residence">New residence </label>
            </div>
            <div>
                <input id="new_residence" v-model="data.input" maxlength="99" type="text" />
            </div>
            <div>
