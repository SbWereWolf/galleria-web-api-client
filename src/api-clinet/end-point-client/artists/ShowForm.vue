...
async function ShowProfile() {
    ...
    const response = await fetch(
        `${apiLocation.value}${endPoint.value}/${data.input}/?session_id=${sessionId}`,
        {
            method: method.value,
            headers: headers,
        },
...
<template>
    <div>
        <form @submit.prevent="ShowProfile">
            <h1>{{ endPoint }}</h1>
            <div>
                <label for="artistId">The first name to search for</label>
            </div>
            <div>
                <input id="artistId" v-model="data.input" maxlength="38" type="number" />
            </div>
            <div>
